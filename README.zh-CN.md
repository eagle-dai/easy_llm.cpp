...
## 更新说明
- `dev/cuda`（CUDA 算子开发分支）已合并到 `release`，目前仍处于刚启动的早期开发阶段。
- `release` 默认保持 CPU 推理：`build.sh` **不**开启 CUDA。如需 CUDA 编译，请在 CMake 中传入 `-DEASY_LLM_ENABLE_CUDA=ON`（并设置 CUDA 架构参数）。
